...
const Home: NextPage = () => {
    const [urlInput, setUrlInput] = useState({message: '', disable: false, listId: ''})
        ...
        fetch(`/api/playlist?playlistId=${listId}`)
            .then(r => {
                    r.json().then(json => {
                        ...
                        setUrlInput({...urlInput, disable: false, message: 'URL contains invalid URL'})
                        setPlaylist([])
                    })
                }
            )
    ...
    const onTimeChange = (timeStr: string) => {
        ...
        window.setInterval(() => {
            ...
            if (date.getHours() === nowDate.getHours() && date.getMinutes() === nowDate.getMinutes() && nowDate.getSeconds() === 0) {
        ...
        <div className={styles.container}>
            ...
            <main className={styles.main}>
                <div className={styles.header}>
                    <p>
                        <Image src="/shuffle_on_FILL1_wght400_GRAD0_opsz48.svg" alt="icon" width="64" height="64"/>{' '}
                        <Image src="/alarm_FILL1_wght400_GRAD0_opsz48.svg" alt="icon" width="64" height="64"/>
                    </p>
                ...
                <div className={styles.grid}>
                    <div>
                        <div className={styles.card}>
                            ...
                            <p>
                                {urlInput.message}
                                <input type="text" placeholder="URL here" onChange={onUrlInputChange}
                                       style={{width: '100%'}}
                        ...
                        <div className={styles.card}>
                            <h2>Player</h2>
                            {!playerSrc ? <hr/> : <iframe width="100%" height="auto" src={playerSrc} frameBorder="0"/>}
                        </div>
                    </div>
                    <div className={styles.card}>
                        <h2>Playlist</h2>
                        {playlist.map((v: any) => (<div key={v.snippet.resourceId.videoId}
                                                        onClick={() => onChangePlayerSrc(v.snippet.resourceId.videoId)}>
                            <table>
                                <tbody>
                                <tr>
                                    <td>
                                        <img src={v.snippet.thumbnails.high.url} width={81} height={60} loading={"lazy"}
                                             alt={'thumbnail image'} style={{marginRight: '12px'}}/>
                                    </td>
                                    <td>
                                        <span>{v.snippet.title}{' '}</span><br/>
                                        <small style={{fontSize: '0.7em'}}>{v.snippet.resourceId.videoId}</small>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
